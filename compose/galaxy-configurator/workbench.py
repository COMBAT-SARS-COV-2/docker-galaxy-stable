...
CONTEXT_SETTINGS = dict(
    default_map={
        'download_jar': {'illumina_version': config('ILLUMINA_VERSION', default='latest'),
...
def download_jar(illumina_version, nanopore_version, access_token):
    ...
    for repo in g.get_organization('combat-sars-cov-2').get_repos():
        ...
        if version:
            ...
            if release is None: raise click.ClickException("Something went wrong: release is not found")
            ...
            for asset in release.get_assets():
                ...
                with tqdm.wrapattr(open(file_name, "wb"), "write",
                                   miniters=1, desc=asset.browser_download_url.split('/')[-1],
